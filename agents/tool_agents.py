...
class ReactAgent:
        ...
        if action == None or action == "" or action == "\n":
            ...
            if action_type == "FlightSearch":
                try:
                    if (
                        ...
                        self.current_data = self.tools["flights"].run(
                            action_arg.split(", ")[0], action_arg.split(", ")[1], action_arg.split(", ")[2]
                        )
                ...
                try:
                    ...
                    self.json_log = "Illegal args. Other Error"
    ...
    def _build_agent_prompt(self) -> str:
        if self.mode == "zero_shot":
            return self.agent_prompt.format(query=self.query, scratchpad=self.scratchpad)
    ...
    def is_halted(self) -> bool:
        return (
            (self.step_n > self.max_steps) or (len(self.enc.encode(self._build_agent_prompt())) > self.max_token_length)
...
if __name__ == "__main__":
    ...
    agent = ReactAgent(
        None, tools=tools_list, max_steps=30, react_llm_name=args.model_name, planner_llm_name=args.model_name
    )
