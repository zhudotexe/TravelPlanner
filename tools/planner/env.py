    ...
    def run(self, tested_data):
        ...
        if "transportation" in unit and unit["transportation"] and unit["transportation"] != "-":
            ...
            if "flight number" in value.lower():
                try:
                    res = self.flight.data[
                        self.flight.data["Flight Number"] == value.split("Flight Number: ")[1].split(",")[0]
                    ]
                    if len(res) > 0:
                ...
                try:
                    ...
                    returned_info.append(
                        "The transporation information is not valid, please check. You have to make sure there are two cities (from A to B) in your transportation plan."
                    )

        if "breakfast" in unit and unit["breakfast"] and unit["breakfast"] != "-":
            name, city = get_valid_name_city(unit["breakfast"])
            if name != "-" and city != "-":
                res = self.restaurants.data[
                    (self.restaurants.data["Name"] == name) & (self.restaurants.data["City"] == city)
                ]
                if len(res) > 0:
        ...
        if "accommodation" in unit and unit["accommodation"] and unit["accommodation"] != "-":
            ...
            if name != "-" and city != "-":
                res = self.accommodation.data[
                    (self.accommodation.data["NAME"] == name) & (self.accommodation.data["city"] == city)
                ]
                if len(res) > 0:
                    total_cost += res["price"].values[0] * math.ceil(
                        people_number * 1.0 / res["maximum occupancy"].values[0]
                    )
                else:
