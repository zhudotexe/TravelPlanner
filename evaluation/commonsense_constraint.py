        ...
        if unit["transportation"] and unit["transportation"] != "-":
            ...
            if "flight number" in value.lower():
                ...
                if (
                    len(
                        flight.data[
                            (flight.data["Flight Number"] == flight_number)
                ...
                if "self-driving" in value.lower():
                    if (
                        googleDistanceMatrix.run_for_evaluation(org_city, dest_city, mode="self-driving")["cost"]
                        == None
                    ):
...
def is_valid_days(question, tested_data):
    ...
    for i in range(min(question["days"], len(tested_data))):
        if (
            tested_data[i] != {}
            and tested_data[i]["current_city"] != "You don't need to fill in the information for this or later days."
        ):
            lens += 1
...
def is_not_absent(question, tested_data):
    ...
    for i in range(min(question["days"], len(tested_data))):
        ...
        if (
            unit["breakfast"] in ["", "-"] or unit["lunch"] in ["", "-"] or unit["dinner"] in ["", "-"]
