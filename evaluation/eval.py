    ...
    for level in commonsense_constraint_record:
        for day in commonsense_constraint_record[level]:
            remap_commonsense_constraint_record[level][day] = {
                mapping_dict[key]: val for key, val in commonsense_constraint_record[level][day].items()
            }
            remap_hard_constraint_record[level][day] = {
                mapping_dict[key]: val for key, val in hard_constraint_record[level][day].items()
            }
...
def eval_score(set_type: str, file_path: str):
    ...
    mapping_constraint_record = {
        key: {
            day: {"valid_room_rule": 0, "valid_cuisine": 0, "valid_room_type": 0, "valid_transportation": 0}
            for day in [3, 5, 7]
        }
        for key in ["medium", "hard"]
    }
    ...
    for idx in range(0, len(query_data_list)):
        if plan_constraint_store[idx]["commonsense_constraint"]:
            ...
            for item in plan_constraint_store[idx]["commonsense_constraint"]:
                if (
                    plan_constraint_store[idx]["commonsense_constraint"][item][0] is not None
                    and not plan_constraint_store[idx]["commonsense_constraint"][item][0]
                ):
