...
def budget_calc(org, dest, days, date: list, people_number=None, local_constraint=None):
    ...
    if grain == "city":
        ...
        for city in city_set:
            if dest == city.split("\t")[1]:
                ...
                current_flight_data = flight.data[
                    (flight.data["DestCityName"] == candidate_city) & (flight.data["OriginCityName"] == org)
                ]
    ...
    if local_constraint:
        ...
        if local_constraint["room type"]:
            if local_constraint["room type"] == "shared room":
                ...
                hotel_data = hotel_data[
                    (hotel_data["room type"] == "Private room") | (hotel_data["room type"] == "Entire home/apt")
                ]
        ...
        if local_constraint["cuisine"]:
            ...
            restaurant_data = restaurant_data[
                restaurant_data["Cuisines"].str.contains("|".join(local_constraint["cuisine"]))
            ]
    ...
    for mode in ["lowest", "highest", "average"]:
        ...
        restaurant_budget = (
            estimate_budget(restaurant_data["Average Cost"].tolist(), mode) * multipliers[days]["restaurant"]
        )
