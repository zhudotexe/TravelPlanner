...
def get_org_dest(days: int):
    if days == 3:
        ...
        while True:
            ...
            if (
                dest != org
                and "None" not in dest
                and dest.split("\t")[1] != org.split("\t")[1]
                and len(state_city_map[dest.split("\t")[1]]) > 3
            ):
                break
        final_org = org.split("\t")[0]
...
def easy_level_element_selection(day_list):
    ...
    query_dict = {
        ...
        "date": date,
        "people_number": 1,
        "local_constraint": local_constrain_record,
        "budget": final_budget,
        "query": None,
        "level": "easy",
    }
...
def medium_level_element_selection(day_list):
    ...
    #     local_constrain_record["rating"] = local_constraint
        ...
        local_constraint = random.sample(
            ["Chinese", "American", "Italian", "Mexican", "Indian", "Mediterranean", "French"], 2
        )
    ...
    if days == 3:
        ...
        final_budget = round_to_hundreds(
            round_to_hundreds((budget["average"] + budget["highest"]) / 2) * people_number * 0.75
        )
...
def hard_level_element_selection(day_list):
    ...
    local_constraint_type_list = np.random.choice(
        local_constraint_list, size=3, replace=False, p=probabilities
    ).tolist()
    ...
    for local_constraint_type in local_constraint_type_list:
        if local_constraint_type == "flight time":
            ...
            # choice_number = random.choice([2,3,4,5])
            local_constraint = random.sample(
                ["Chinese", "American", "Italian", "Mexican", "Indian", "Mediterranean", "French"], 4
            )
            local_constrain_record["cuisine"] = local_constraint

    budget = budget_calc(
        final_org, final_des, days=days, date=date, people_number=people_number, local_constraint=local_constrain_record
    )
    ...
    if days == 3:
        ...
        final_budget = round_to_hundreds(
            round_to_hundreds((budget["average"] + budget["highest"]) / 2) * people_number * 0.5
        )
